...
# Aria's farewell phrases when summoning maids
ARIA_FAREWELLS = {
    "sophia": [
        "I'll have Sophia look into that.",
        "Sophia will handle this.",
        "Let me get Sophia for you.",
    ],
    "luna": [
        "Luna will take care of the entertainment.",
        "Let me get Luna for you.",
        "Luna's your girl for this.",
    ],
    "rose": [
        "Rose will organize this.",
        "Let me get Rose for you.",
        "Rose handles all scheduling.",
    ],
    "mei": [
        "Mei will handle the devices.",
        "Let me get Mei.",
        "Mei's on it.",
    ],
    "clara": [
        "Clara will help with that.",
        "Let me get Clara for you.",
        "Clara's perfect for this.",
    ],
}


def _get_aria_farewell(maid_name: str) -> str:
    """Get a random farewell phrase for Aria when summoning a maid."""
    phrases = ARIA_FAREWELLS.get(maid_name.lower(), [f"Let me get {maid_name} for you."])
    return random.choice(phrases)


async def _speak_farewell(context: RunContext, maid_name: str) -> None:
    """Have Aria speak her farewell before handoff using session.say()."""
    try:
...
async def summon_sophia(context: RunContext):
    ...
    await _speak_farewell(context, "sophia")
...
async def summon_luna(context: RunContext):
    ...
    await _speak_farewell(context, "luna")
...
async def summon_rose(context: RunContext):
    ...
    await _speak_farewell(context, "rose")
...
async def summon_mei(context: RunContext):
    ...
    await _speak_farewell(context, "mei")
...
async def summon_clara(context: RunContext):
    ...
    await _speak_farewell(context, "clara")
...
async def summon_maid_by_name(context: RunContext, maid_name: str):
    ...
    await _speak_farewell(context, maid_name_lower)
...
async def suggest_and_summon_maid(context: RunContext, task_description: str):
    ...
    await _speak_farewell(context, suggested)
